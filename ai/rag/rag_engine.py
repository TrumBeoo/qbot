...
class RAGEngine:
    ...
    def search_similar(self, query: str, k: int = 4) -> List[Document]:
        """Tim cac doan tai lieu gan truy van nhat, KHONG goi LLM.

        Truoc day endpoint /search-similar trong app.py tu goi
        self._load_vectorstore().similarity_search(...) - tang HTTP choc vao
        method private cua engine. Dua len thanh API public de cho goi khong
        phai biet vectorstore duoc nap kieu gi.
        """
        return self._load_vectorstore().similarity_search(query, k=k)
