...
def search_similar():
    ...
    try:
        ...
        from RAG.rag_engine import get_rag_engine
        # search_similar() thay cho viec choc vao _load_vectorstore() cua ban
        # cu, va tra ve list dict thay vi Document cua langchain.
        hits = get_rag_engine().search_similar(query, k=5)
...
# Tinh nang memory da bo: 8 endpoint (/chat-with-memory va /api/memory/*)
# khong co client nao goi - kiem bang grep toan bo fe/src va Dashboard/src.
# Ba service dang sau (memory_service, enhanced_rag_service,
# enhanced_chat_service, 1.402 dong) cung da xoa. Chung la lop dieu phoi
# DB + langchain, thuoc pha 04 chu khong phai AI service.
