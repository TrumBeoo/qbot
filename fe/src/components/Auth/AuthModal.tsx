...
import type { AuthOutcome } from '../../types/api';
import {
  Modal,
  ModalOverlay,
  ModalContent,
...
const validateForm = (formData, authMode, language) => {
  // Record<string, string>: key la ten field, value la thong diep loi.
...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onLogin: (email: string, password: string) => Promise<AuthOutcome>;
  onRegister: (name: string, email: string, password: string) => Promise<AuthOutcome>;
  onSocialLogin: (provider: string, token: string) => Promise<AuthOutcome>;
  language: string;
  initialMode?: 'login' | 'register';
}

// Da bo prop `onLanguageChange`: Sidebar khong truyen no, AuthModal chi
// chuyen tiep xuong SocialLoginButtons, va SocialLoginButtons khong khai bao
// no. Plumbing chet di qua ba tang.
const AuthModal = ({
  isOpen,
  onClose,
  onLogin,
  ...
  // Record<string, string>: key la ten field, value la thong diep loi.
  // useState({}) khong co type param thi TS suy ra `{}` va moi lan doc
  // formErrors.email deu bao loi.
