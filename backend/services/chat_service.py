...
class ChatService:
    ...
    def create_conversation(cls, user_id, title="New Conversation"):
        ...
        conversation = ChatRepository.create_conversation(user_id, title.strip())
        return conversation.to_dict(include_messages=False)
    
    @classmethod
    def get_user_conversations(cls, user_id, limit=50, skip=0):
        """Get all conversations for a user"""
        conversations = ChatRepository.find_conversations_by_user(user_id, limit, skip)
    ...
    def get_conversation(cls, conversation_id, user_id):
        ...
        conversation = ChatRepository.find_conversation_by_id(conversation_id, user_id)
        if not conversation:
    ...
    def update_conversation_title(cls, conversation_id, user_id, title):
        ...
        success = ChatRepository.update_conversation(
            conversation_id, 
            user_id, 
            {'title': title.strip()}
        )
    ...
    def add_message_to_conversation(cls, conversation_id, user_id, user_message, bot_response=None, language='vi'):
        ...
        # User message
        user_msg = Message(user_message.strip(), 'user', language, timestamp)
        messages.append(user_msg)
        
        # Bot response (if provided)
        if bot_response and bot_response.strip():
            bot_msg = Message(bot_response.strip(), 'bot', language, timestamp)
            messages.append(bot_msg)
        
        # Add to conversation
        success = ChatRepository.add_messages_to_conversation(
            conversation_id, 
            user_id, 
            messages
        )
    ...
    def delete_conversation(cls, conversation_id, user_id):
        ...
        success = ChatRepository.delete_conversation(conversation_id, user_id)
        if not success:
    ...
    def delete_message(cls, conversation_id, user_id, message_id):
        ...
        success = ChatRepository.delete_message(conversation_id, user_id, message_id)
        if not success:
    ...
    def search_conversations(cls, user_id, query, limit=20):
        ...
        conversations = ChatRepository.search_conversations(user_id, query.strip(), limit)
    ...
    def export_user_conversations(cls, user_id):
        ...
        conversations = ChatRepository.export_user_conversations(user_id)
    ...
    def get_conversation_stats(cls, user_id):
        ...
        conversations = ChatRepository.find_conversations_by_user(user_id, limit=1000)
